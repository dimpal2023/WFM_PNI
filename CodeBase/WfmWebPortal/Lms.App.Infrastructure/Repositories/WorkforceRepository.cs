    ...
    public class WorkforceRepository : IWorkforceRepository
    {
            ...
            try
            {
                if (Obj != null)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(Configurations.SqlConnectionString))
                    {
                        ...
                        string sqlQuery = $"select(select Permission from [dbo].[fn_CalPermission] (InTime, OutTime, BreakStartTime, BreakEndTime, OverTime, EarlyBy, LateBy, PunchRecords,bm.BIOMETRIC_CODE)) as p1 from TAB_BIOMETRIC as bm inner join TAB_SHIFT_MASTER as sm on sm.ID = bm.SHIFT_ID where BIOMETRIC_CODE = '{wfReord.BIOMETRIC_CODE}' and ATTENDANCE_DATE='{attDate}'AND STATUS_CODE in ('P','½P','HP','WOP','WO½P','H½P')";
                    ...
                    else
                    {
                        ...
                        if (TotalWorkDur > 270 && TotalWorkDur < 690)
                        {
                            ...
                            else if (Duration == 0 && Convert.ToInt32(BioMetricDetails.OverTime) > 0)
                            {
        ...
        public object AddDailyWork(AddWorkForceItemMetaData dailywork)
        {
            ...
            try
            {
                ...
                else
                {
                        ...
                        else if (Duration == 0 && Convert.ToInt32(dailywork.OverTime) > 0)
                        {
                            TotalWorkDur = TotalWorkDur;
                        }
                        else
        ...
        public List<SerchDailyWorkMetaData> GetDailyWorks(FilterDailyWork filterDailyWork)
        {
            ...
            List<SerchDailyWorkMetaData> result = (from dw in _appEntity.TAB_WORKFORCE_DAILYWORK
                                                   ...
                                                   group new { dw, wf, op } by new { dw.WORK_DATE, wf.WF_ID, wf.EMP_NAME,wf.BUILDING_ID } into grp
                                                   select new SerchDailyWorkMetaData
                                                   {
                                                       ...
                                                       BUILDING_ID = grp.Key.BUILDING_ID,
                                                       ...
                                                       Select(x => new PartialDailyWorkMetaData
                                                       {
                                                           ...
                                                           MachineNo=x.dw.MachineNo,
                                                           AvgPercentage=x.dw.AvgPercentage.ToString(),
                                                           WASTE= x.dw.WASTE.ToString(),
                                                           REJECTION_ON_FINISHING= x.dw.REJECTION_ON_FINISHING.ToString(),
                                                           REJECTION_ON_LOOM= x.dw.REJECTION_ON_LOOM.ToString(),
                                                       }).ToList()
                                                   }
                        ).ToList();
