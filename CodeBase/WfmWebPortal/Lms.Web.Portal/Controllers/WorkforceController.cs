...
namespace Lms.Web.Portal.Controllers
{
    ...
    public class WorkforceController : Controller
    {
        ...
        public WorkforceController(IBaseBL baseBl)
        {
            this.baseBl = baseBl;
        }
        ...
        public ActionResult GetSpecialAllowanceByDept(Guid? deptId, Guid? sub_dept_id, int? emptype_id, int? year_id, Guid? BUILDING_ID2)
        {
            List<SpecialAllowanceMetaData> specialAllowances = new List<SpecialAllowanceMetaData>();
            specialAllowances = this.baseBl.WorkforceBL.GetSpecialAllowanceByDept(deptId, sub_dept_id, emptype_id, year_id, BUILDING_ID2);
        ...
        public JsonResult LoadWorkforceByWFType(Guid deptId, Guid? sub_dept_id, string query, int? emp_type_id)
        {
            ...
            try
            {
                result = baseBl.WorkforceBL.GetEmpAllItems(deptId, sub_dept_id, emp_type_id, null).Where(x => x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
        ...
        public JsonResult LoadWorkforceByPIB(Guid deptId, Guid? sub_dept_id, string query, int? emp_type_id)
        {
            List<WorkforceMetaDataList> result = new List<WorkforceMetaDataList>();
            query = query.ToUpper();
            try
            {
                result = baseBl.WorkforceBL.GetEmpAllItems_PIB(deptId, sub_dept_id, emp_type_id, null).Where(x => x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
                var list = (from q in result
                            select new { Name = string.Concat(q.EMP_NAME, " - ", q.EMP_ID), ID = q.WF_ID }).ToList();
        ...
        [Authorize]
        [HttpGet]
        public ActionResult EditDailyWork(Guid? id, Guid? DW_ID)
        {
            WorkforceDailyWorkMetaData metaData = new WorkforceDailyWorkMetaData();
            Guid companyId = SessionHelper.Get<Guid>("CompanyId");
            var deptList = this.baseBl.DepartmentBL.GetDepartmentByCompanyId(companyId);
            ...
            metaData.BUILDING_ID = metaData.BUILDING_ID;
            ...
            ViewBag.Buildings = new SelectList(baseBl.ManPowerRequestBL.GetBuildings(), "BUILDING_ID", "BUILDING_NAME");
            //Guid BUILDING_ID, Guid deptId, int? emp_type_id, Guid? sub_dept_id
            ViewBag.EmpName = new SelectList(baseBl.WorkforceBL.BindWorkforceByWFType_New(metaData.BUILDING_ID, metaData.DEPARTMENT_ID, metaData.SUBDEPT_ID, metaData.WF_EMP_TYPE, 0), "WF_ID", "EMP_NAME");
            ViewBag.ItemName = new SelectList(baseBl.WorkforceBL.GetItemByDeptId((Guid)(metaData.DEPARTMENT_ID)), "ITEM_NAME", "ITEM_NAME");
            string[] values = metaData.ITEM_NAME.Split('_');
            ViewBag.OperationName = new SelectList(baseBl.WorkforceBL.GetItemOperationsByItemId(values[0].Trim().ToString(), values[1].Trim().ToString(), (Guid)(metaData.DEPARTMENT_ID)), "UNIQUE_OPERATION_ID", "OPERATION");
            ViewBag.ID = id.Value;
            if (id == null)
            {
        ...
        public ActionResult UpdateDailyWork(Guid? UNIQUE_OPERATION_ID, string QTY, Guid? ID,string MachineNo,string AvgPercentage,string WASTE, string REJECTION_ON_LOOM, string REJECTION_ON_FINISHING)
        {
            var redirectUrl = new UrlHelper(Request.RequestContext).Action("SearchDailyWorkList", "WorkManagement");
            try
            {
                baseBl.WorkforceBL.UpdateDailyWork(UNIQUE_OPERATION_ID, QTY, ID, MachineNo, AvgPercentage, WASTE, REJECTION_ON_LOOM, REJECTION_ON_FINISHING);
        ...
        public JsonResult PiecWagerWorkforceByWFType(Guid deptId, Guid? sub_dept_id, string query, int? emp_type_id)
        {
            ...
            try
            {
                result = baseBl.WorkforceBL.GetPiecWagerWorkforceByWFTypes(deptId, sub_dept_id, emp_type_id).Where(x => x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
                var list = (from q in result
                            select new { Name = string.Concat(q.EMP_NAME, " - ", q.EMP_ID), ID = q.WF_ID }).ToList();
        ...
        [HttpGet]
        public JsonResult GetOperationsByitemId(string ItemId, string ItemName,Guid DEPARTMENT_ID)
        {
            //WorkforceBL wfBL = new WorkforceBL();

            return Json(baseBl.WorkforceBL.GetItemOperationsByItemId(ItemId, ItemName, DEPARTMENT_ID), JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult LoadWorkforceAndWFId(Guid deptId, string query)
        {
            ...
            try
            {
                ...
                result = baseBl.WorkforceBL.GetEmpAllItems(companyId, deptId, null, null).Where(x => x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
        ...
        public ActionResult Create([Bind(Exclude = "APPROVED, STATUS, NAME")] WorkforceMetaData workforce, HttpPostedFileBase PHOTO)
        {
            ...
            workforce.SIGNATUREfile = workforce.PHOTOfile;
            ...
            //}
            var redirectUrl1 = new UrlHelper(Request.RequestContext).Action("AllItems", "Workforce");
            return Json(new { Url = redirectUrl1, id = "1" });
        }
        ...
        [HttpGet]
        public bool IsBiometricAvailable1(string BIOMETRIC_CODE, Int64? AADHAR_NO)
        {
            return baseBl.WorkforceBL.IsBiometricAvailable(BIOMETRIC_CODE, AADHAR_NO);
        }
        ...
        [HttpGet]
        public JsonResult IS_EMPID_EXIST(string EMP_ID, Int64? AADHAR_NO)
     {
            return Json(!baseBl.WorkforceBL.IS_EMPID_EXIST(EMP_ID, AADHAR_NO), JsonRequestBehavior.AllowGet);
        }
        public bool IS_EMPID_EXIST1(string EMP_ID, Int64? AADHAR_NO)
        {
            return baseBl.WorkforceBL.IS_EMPID_EXIST(EMP_ID, AADHAR_NO);
        }
        ...
        public ActionResult Salary()
        {
            ...
            List<BankMasterMetaData> bank = baseBl.MasterDataBL.GetAllBank();
            ViewBag.BANK = new SelectList(bank, "BANK_ID", "BANK_NAME");
            ViewBag.MyQSVal = Request.QueryString["wfid"];
            return View(data);
        }
        [HttpGet]
        ...
        [HttpGet]
        public ActionResult SelectBankData(string BANK_ID)
        {
            List<BankMasterMetaData> result = new List<BankMasterMetaData>();
            Guid BANK_IDS = new Guid(BANK_ID);
            result = baseBl.MasterDataBL.SelectBankData(BANK_IDS);
          
            return Json(result, JsonRequestBehavior.AllowGet);

        }
        [HttpGet]
        public ActionResult CheckUAN(string UAN_NO)
        {
            ExportSalaryMetaData result = baseBl.MasterDataBL.CheckUAN(UAN_NO);
            if (result == null)
            {
                return Json("NotFound", JsonRequestBehavior.AllowGet);
            }
        ...
        {
            ExportSalaryMetaData result = baseBl.MasterDataBL.CheckPAN(PAN);
            if (result == null)
            {
                return Json("NotFound", JsonRequestBehavior.AllowGet);
            }
        ...
        [HttpGet]
        public ActionResult CheckEPF(string EPF)
        {
            ExportSalaryMetaData result = baseBl.MasterDataBL.CheckEPF(EPF);
            if (result == null)
            {
                return Json("NotFound", JsonRequestBehavior.AllowGet);
            }
        ...
        [HttpGet]
        public ActionResult CheckESIC(string ESIC)
        {
            ExportSalaryMetaData result = baseBl.MasterDataBL.CheckESIC(ESIC);
            if (result == null)
            {
                return Json("NotFound", JsonRequestBehavior.AllowGet);
            }
        ...
        [HttpGet]
        public ActionResult CheckAccountNo(string ACC)
        {
            ExportSalaryMetaData result = baseBl.MasterDataBL.CheckAccountNo(ACC);
            if (result == null)
            {
                return Json("NotFound", JsonRequestBehavior.AllowGet);
            }
        ...
        public JsonResult LoadWorkforce(Guid deptId, string query)
        {
            ...
            try
            {
                ...
                else
                {
                    AccountValidateUser_Result loggedin_user = Session["USER"] as AccountValidateUser_Result;
                    Guid companyId = loggedin_user.COMPANY_ID;
                    result = baseBl.WorkforceBL.GetEmpAllItems(companyId, deptId, null, null).Where(x => x.DEPT_ID == deptId && x.EMP_NAME.ToUpper().Contains(query) || x.EMP_ID.ToUpper().Contains(query)).ToList();
                }
        ...
        public ActionResult Salary([Bind(Exclude = "APPROVED, STATUS, NAME")] WorkforceSalaryMetaData salary)
        {
            ...
            var redirectUrl = new UrlHelper(Request.RequestContext).Action("Salary", "Workforce");
            return Json(new { Url = redirectUrl, ID = "1" });
        ...
        public ActionResult GetEmployessforAddSalary(Guid? deptId, Guid? sub_dept_id, int? emptype_id, Guid? BUILDING_ID)
        {
            WorkflowMasterVieweMetaData metaData = new WorkflowMasterVieweMetaData();
            List<WorkforceMetaDataList> employees = this.baseBl.WorkforceBL.GetEmployessforAddSalary(deptId, sub_dept_id, emptype_id, BUILDING_ID);
            return PartialView("_EmployeesSalary", employees);
        }
        //public ActionResult ExportEmployee(Guid? deptId, Guid? sub_dept_id, int? emptype_id, Guid? BUILDING_ID)
        //{
        //    WorkflowMasterVieweMetaData metaData = new WorkflowMasterVieweMetaData();
        //    List<WorkforceMetaDataList> employees = this.baseBl.WorkforceBL.ExportEmployee(deptId, sub_dept_id, emptype_id, BUILDING_ID);
        //    return PartialView("_EmployeesSalary", employees);
        //}
        ...
        public ActionResult Edit(Guid? wfid)
        {
            ...
            var deptList = baseBl.DepartmentBL.GetDepartmentByCompanyId_Admin(companyid,wm.BUILDING_ID).Select(x =>
                                  new SelectListItem()
                                  {
                                      Text = x.DEPT_NAME.ToString(),
                                      Value = x.DEPT_ID.ToString()
        ...
        {
            if (IS_EMPID_EXIST1(workforce.EMP_ID, workforce.AADHAR_NO) == true)
            {
                return Json(new { id = "3" });
            }
            if (IsBiometricAvailable1(workforce.BIOMETRIC_CODE, workforce.AADHAR_NO) == true)
            {
        ...
        public JsonResult GetItemsBydeptId_New(Guid deptId, string query)
        {
            List<ItemMasterMetaData> result = new List<ItemMasterMetaData>();
            query = query.ToUpper();
            try
            {
                AccountValidateUser_Result loggedin_user = Session["USER"] as AccountValidateUser_Result;
                result = baseBl.WorkforceBL.GetItemByDeptId(deptId).Where(x => (x.ITEM_CODE_NAME+"_"+ x.ITEM_NAME).ToUpper().Contains(query)).ToList();

                var list = (from q in result
                            select new { Name = q.ITEM_CODE_NAME + "_" + q.ITEM_NAME, ID = q.ITEM_CODE_NAME + "_" + q.ITEM_NAME }).Select(x=>x.Name).Distinct().ToList();
